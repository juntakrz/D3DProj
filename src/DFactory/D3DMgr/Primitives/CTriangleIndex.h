...
public:
	...
	CTriangleIndex() = default;
	CTriangleIndex(std::vector<T> inVerts, std::vector<uint32_t> inIndices)
		:
	...
	void SetTangentBinormalNormal() noexcept
	{
		...
		for (uint32_t i = 0; i < indices.size(); i += 3)
		{
			...
			const DirectX::XMVECTOR vecT = DirectX::XMVector3Normalize(DirectX::XMLoadFloat3(&tangent));
			const DirectX::XMVECTOR vecB = DirectX::XMVector3Normalize(DirectX::XMLoadFloat3(&binormal));
	...
	void SetNormalsTri() noexcept
	{
		...
		for (uint32_t i = 0; i < indices.size(); i += 3)
		{
			...
			//cross product of triangle's side vectors calculated by connecting vertices by vector
			const auto normal = DirectX::XMVector3Normalize(DirectX::XMVector3Cross((vec1 - vec0), (vec2 - vec0)));
