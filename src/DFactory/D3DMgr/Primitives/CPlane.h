...
public:
	template<class V>
	static CTriangleIndex<V> Create(const uint16_t divX, const uint16_t divY)
	{
		ASSERT(divX > 0);
		ASSERT(divY > 0);

		constexpr float coord = 2.0f;
		float stepX = coord / divX;
		float stepY = coord / divY;
	...
	template <class V>
	static CTriangleIndex<V> Create(uint16_t divisions)
	{
