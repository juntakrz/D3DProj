...
void DFactory::CProc::ProcessCamera() noexcept
{
	...
	pKbd->IsKeyDown('W') ? m_camDir.z += 1.0f : 0.0f;
	pKbd->IsKeyDown('S') ? m_camDir.z += -1.0f : 0.0f;
	pKbd->IsKeyDown('A') ? m_camDir.x += -1.0f : 0.0f;
	pKbd->IsKeyDown('D') ? m_camDir.x += 1.0f : 0.0f;
	pKbd->IsKeyDown('R') ? m_camDir.y += 1.0f : 0.0f;
	pKbd->IsKeyDown('F') ? m_camDir.y += -1.0f : 0.0f;
